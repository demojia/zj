...
//ZAOption.fn = ZAOption.prototype = {
  ...
  ZAOption.login = function(callback){
	...
	if(!memberId || memberId < 1){
		...
		seajs.use("http://images.zastatic.com/zhenai3/zhenai2012/css/layer/login.css",function(){
  ...
  ZAOption.login2 = function(callback){
	...
	if(!memberId || memberId < 1){
		...
		seajs.use("http://images.zastatic.com/zhenai3/zhenai2012/css/layer/login.css",function(){
			ZAOption.asyncAjax("/commons/login.jsps",{}, function(data){
				var dialog = art.dialog({
					id:"login",
					title:"",
					lock:true,
		...
		if(data.code == 0){
			...
			if(hide){
				art.dialog({
					...
					lock: true,
					ok:function(){
						$.getJSON('/v2/profile/publicMyInfo.do',function(d){
							if(d.code == 1){
 ...
 /**
  ...
  ZAOption.sendLeer = function(memberId,skipPhoto,callback,leerSource,optionType){
	...
	$.getJSON('/v2/profile/caninteractRestrict.do?memberId='+memberId,function(data){
		if(data.code == 0){
			...
			seajs.use("http://images.zastatic.com/zhenai3/zhenai2012/css/layer/popmod.css",function(){
  ...
  ZAOption.sendCard = function(memberId,back,callback,optionType){
	...
	var arg = arguments;
	ZAOption.ajax("http://profile.zhenai.com/commons/checkSendCard.jsps", {"memberId":memberId,"callingCard.back":back,"optionType":optionType},function(data){
		if(!data){return;}
		switch(data.code){
		case 0:
			ZA.msgTip(data.msg);
  ...
  ZAOption.sendGift = function(memberId,config,roseFrom){
	...
	if(dialog){
		...
		seajs.use("http://images.zastatic.com/zhenai3/zhenai2012/css/layer/pop_gift.css",function(){
  ...
  */
  ...
  ZAOption.giftPay = function(memberId,typeId,quantity,batchInfo,callback,skipTicket,related,payByCoinFlag,giftEnter,roseFrom){
	...
	//赠送
	ZAOption.asyncAjax("/commons/sendGiftPayView.jsps",{"memberId":memberId,"rosevo.typeId":typeId,"rosevo.quantity":quantity,"rosevo.batchInfo":batchInfo,"skipTicket":skipTicket,"rosevo.related":related,"payFrom":payFrom,"payByCoinFlag":payByCoinFlag,"giftEnter":giftEnter,"roseFrom":roseFrom},function(data){
		...
		case 3:
			...
			$.ajax({
				...
				dataType: "json",
				async:false,
				url: "/personal/createTotalRechargeOrder.jsps",
				...
				success: function(data){
					...
					case 3://没有余额
						...
						$.ajax({
							...
							dataType: "json",
							async:false,
							url: "/payment/createPayOrder.jsps",
  ...
  ZAOption.sendGiftMsg = function(objMemberId) {
	  ...
	  $.ajax({
	   	  ...
	   	  dataType: "json",
	   	  async:false,
	   	  url: "/commons/sendGiftXinShi.jsps",
  ...
  ZAOption.lookCard = function(memberId,keep,sendIdCard,optionType){
	...
	seajs.use(["http://images.zastatic.com/zhenai3/zhenai2012/css/layer/approve_idpup.css","http://images.zastatic.com/zhenai3/zhenai2012/css/layer/approve_info.css"],function(){
		...
		if(data.code == 0){
			...
			ZAOption.ajax("http://profile.zhenai.com/commons/contact.jsps",{"memberId":memberId,"skipPhoto":skipPhoto,"optionType":optionType},function(data){
				...
				function freeMailBack(){
					seajs.use(["http://images.zastatic.com/zhenai3/zhenai2012/css/layer/matchmaker_c.css","artDialog"],function(){
  ...
  ZAOption.chat = function(memberId, objectId) {
	  ...
	  var result = $.ajax({
			url:'/v2/profile/caninteractRestrict.do',
		...
		if(data.code == 0){
			...
			return false;
			 ...
			 ZAOption.asyncAjax("/im/checkChat.jsps", {"objectid" : objectId}, function(data) {
				  if (!data) {
					  return;
				  }
				  switch(data.code) {
