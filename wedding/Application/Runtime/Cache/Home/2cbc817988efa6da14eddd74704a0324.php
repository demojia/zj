...
<div style="clear:none" class="skin-content posr">
	<div style="width:100%;background:#feeeed;">
		<form action="<?php echo U('Personal/dophoto');?>" method="post" enctype="multipart/form-data">
...
<?php if(is_array($list)): foreach($list as $key=>$val): ?><div class="ppic" style="margin-left:10px;float:left;">
		<img id="imgs" src="/wedding/Public/photo/<?php echo ($val["picname"]); ?>" alt="<?php echo ($val["picname"]); ?>">
			<div>
				<button class="del btn-danger">删除照片</button>
                <input type="checkbox" class="box" value="<?php echo ($val["picname"]); ?>">
			</div>
...
<script>
	// //删除相册中的图片
     $('.del').click(function(){
        ...
        $.ajax({
            type:'post',
            url:"<?php echo U('Personal/delper');?>",
...
<script type="text/javascript">
        var list=document.getElementsByClassName('box');
            ...
            $('.shanchu').click(function(){
                ...
                $.ajax({
                    type:'post',
                    url:'<?php echo U("Personal/delAll");?>',
                    data:'picname='+tu,
                    success:function(msg)
                 {
